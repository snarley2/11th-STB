...
[
    {(getPosATLVisual (_this select 0) select 2) <= (_this select 3)},
    {
        ...
        // If velocity is 0 at chute height it will trigger a landing
        private _zVelocity = if (MEU_HEV_Chute_Velocity == 0) then { -1 } else { MEU_HEV_Chute_Velocity };

        [_hev,[0,0,_zVelocity]] remoteExecCall ["setVelocityModelSpace",_hev];
