...
[
	...
	{((getPosATL (_this select 0)) select 2 < MEU_HEV_Landed_Altitude) OR {((getPosATL (_this select 0)) select 2 < (_this select 1)) AND ((velocity (_this select 0)) select 2 isEqualTo 0)} OR {(getPosASLW (_this select 0) select 2) <= 3}},
	{
		...
		if (MEU_HEV_Explosion_Enabled) then
		{
			[_hev, false] remoteExecCall ["enableSimulationGlobal", 2];
			
			if (isClass (configFile >> "CfgAmmo" >> MEU_HEV_Explosion_Class)) then
			{
				private _exp = createVehicle [MEU_HEV_Explosion_Class, position _hev, [], 0, "CAN_COLLIDE"]; 
				_exp setDamage 1;
			};
			...
			[
				{
					...
					[_hev, true] remoteExecCall ["enableSimulationGlobal", 2];
				},
				[_hev],
