...
private _startHeights = ((MEU_HEV_Start_Heights splitString ",") apply { parseNumber _x }) - [0];

if (count _startHeights == 0) then
{
	_startHeights = [5000, 4500, 4000];
};
...
private _atmosphereStart = ((MEU_HEV_Atmosphere_Start splitString ",") apply { parseNumber _x }) - [0];

if (count _atmosphereStart == 0) then
{
	_atmosphereStart = [3000];
};

_atmosphereStart apply {
	_i = _11 lbAdd (format ["Atmosphere Start: %1m",_x]);
	_11 lbSetValue [_i, _x];
};

private _atmosphereEnd = ((MEU_HEV_Atmosphere_End splitString ",") apply { parseNumber _x }) - [0];

if (count _atmosphereEnd == 0) then
{
	_atmosphereEnd = [2000];
};

_atmosphereEnd apply {
	_i = _12 lbAdd (format ["Atmosphere End: %1m",_x]);
	_12 lbSetValue [_i, _x];
};

private _chuteOpen = ((MEU_HEV_Chute_Open splitString ",") apply { parseNumber _x }) - [0];

if (count _chuteOpen == 0) then
{
	_chuteOpen = [1000,500,300];
};
...
private _chuteDetach = ((MEU_HEV_Chute_Detach splitString ",") apply { parseNumber _x }) - [0];

if (count _chuteDetach == 0) then
{
	_chuteDetach = [500,200,100];
};
